...
VITE_ALLOWED_EMAIL="your-admin-email@example.com"
```
...
# Application Configuration
ALLOWED_ADMIN_EMAIL="your-admin-email@example.com" # Must match VITE_ALLOWED_EMAIL for frontend access validation
...
*   `ALLOWED_ADMIN_EMAIL`: This **must** be the same email address you use for `VITE_ALLOWED_EMAIL` in the frontend's `.env` file.
...
3.  **Set Firebase Rules:**
    The `database.rules.json` and `storage.rules` in the root of the project are deployed with `firebase deploy`.
    You **MUST** manually edit `database.rules.json` and `storage.rules` to replace the placeholder email `'YOUR_ADMIN_EMAIL_CONFIGURED_IN_FUNCTIONS_ENV'` with the actual email you configured in `functions/.env` for `ALLOWED_ADMIN_EMAIL`.
    Example for `database.rules.json`:
    ```json
    {
      "rules": {
        "reports": {
          ".read": "auth != null && auth.token.email === 'your-admin-email@example.com'",
          ".write": "auth != null && auth.token.email === 'your-admin-email@example.com'",
          "$reportId": {
            // Allow read/write for specific report if user is the admin
          }
        }
      }
    }
    ```
    After editing, deploy them:
    ```bash
