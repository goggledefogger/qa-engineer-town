    ...
    <Card title="AI UX & Design Insights" className="font-sans">
      {aiUxDesignSuggestions.modelUsed && (
        <p className="text-xs sm:text-sm text-slate-500 mb-2 sm:mb-3">
          Insights generated by: {aiUxDesignSuggestions.modelUsed}
        </p>
      )}
      {aiUxDesignSuggestions.introductionText && aiUxDesignSuggestions.introductionText.trim() !== "" && (
        <div className="prose prose-sm sm:prose-base max-w-none mb-3 sm:mb-4 text-slate-700 prose-p:leading-relaxed">
      ...
      <ul className="space-y-3 sm:space-y-4 list-none p-0">
        {aiUxDesignSuggestions.suggestions.map((item, index) => (
          <li
            key={index}
            className="p-3 sm:p-4 bg-slate-50 rounded-md shadow-sm border border-slate-200"
          >
            {item.screenContext && (
              <p className="text-xs sm:text-sm text-sky-700 font-medium mb-1 capitalize">
                Context: {item.screenContext}
              </p>
            )}
            <div className="prose prose-sm sm:prose-base max-w-none text-slate-800 prose-p:font-medium prose-p:mb-1.5 prose-p:leading-snug">
              <ReactMarkdown remarkPlugins={[remarkGfm]}>
                {unwrapMarkdown(item.suggestion)}
              </ReactMarkdown>
            </div>
            {item.reasoning && item.reasoning.trim() !== "" && (
              <div className="mt-2 prose prose-xs sm:prose-sm max-w-none text-slate-600 pl-2 pt-1 pb-1 border-l-2 border-slate-300 prose-p:leading-relaxed prose-p:italic prose-strong:font-semibold prose-strong:not-italic">
                <ReactMarkdown remarkPlugins={[remarkGfm]}>
